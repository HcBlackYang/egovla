...
def train_stage_c(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(loader):
            ...
            # === Modality Dropout (核心逻辑修正) ===
            # 我们直接在 video tensor 上操作
            # 50% 概率将 Main Camera (View 0) 抹黑
            if torch.rand(1) < 0.5:
                 video[:, 0] = 0.0 
            
            # 确保 First Frame 也是双摄格式 (Dataset 应该已经处理好了，如果是 [B, 2, 3, H, W])
            # 如果 Dataset 返回的 ff 是 [B, 2, 3, H, W]，则需要 unsqueeze 时间维度适配 Encoder 接口
            # FusionEncoder 期望 ff 是 [B, 2, 3, H, W] (如果 forward 内部没有处理 T) 
            # 或者 [B, 2, 3, 1, H, W]。
            # 查看 FusionEncoder 代码，它会 repeat T_video 次，所以 [B, 2, 3, H, W] 即可。
            # 但为了保险，如果 ff 是 5D [B, 2, 3, H, W]，Encoder 内部处理逻辑要对。
            # 您的 Dataset Loader 返回的是 [2, 3, H, W] stacked -> Batch -> [B, 2, 3, H, W]。正确。
            ...
            with autocast('cuda', dtype=torch.bfloat16):
                ...
                # 3. Consistency Loss (已启用)
                # 这会计算 "Single View Feature" 和 "Dual View Feature" 之间的 MSE
            ...
            if i % 10 == 0:
                print(f"Epoch {epoch} [{i}] Diff: {loss_diff.item():.4f} Cons: {loss_cons.item():.4f}")
