# # ego纯数据项目
# import sys
# import os
# import time
# import logging
# import cv2
# import numpy as np
# import math
# import threading
# from collections import deque
# from common.constants import ActionSpace
# from robots.franky_env import FrankyEnv
# from robots.robot_param import RobotParam
# from systems.tcp_client import TCPClientPolicy 
# from cameras.realsense_env import RealSenseEnv

# # 配置日志格式
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# class ImageRecorder(threading.Thread):
#     def __init__(self, camera, buffer_size=16):
#         super().__init__()
#         self.camera = camera
#         self.buffer_size = buffer_size
#         self.running = False
#         self.lock = threading.Lock()
        
#         self.latest_frame = None
#         self.frame_buffer = deque(maxlen=buffer_size) 
#         self.stop_event = threading.Event()
        
#         # 录制控制标志位
#         self.is_recording = True 

#     def run(self):
#         self.running = True
#         self.camera.start_monitoring()
#         logging.info("[ImageRecorder] Background thread started.")
        
#         while not self.stop_event.is_set():
#             data = self.camera.get_latest_frame()
#             if data is not None:
#                 img = data['bgr']
                
#                 # 只有在“录制模式”下才更新 Buffer 和 Latest Frame
#                 if self.is_recording:
#                     with self.lock:
#                         self.latest_frame = img.copy()
#                         self.frame_buffer.append(img)
                
#                 # 实时显示保持常开
#                 cv2.imshow("Wrist View (Real-time)", img)
#                 if cv2.waitKey(1) & 0xFF == ord('q'):
#                     self.stop_event.set()
            
#             time.sleep(0.033) 
        
#         cv2.destroyAllWindows()
#         logging.info("[ImageRecorder] Stopped.")

#     def get_latest_image(self):
#         with self.lock:
#             if self.latest_frame is None: return None
#             return self.latest_frame.copy()

#     def pause_recording(self):
#         """暂停更新 Buffer"""
#         self.is_recording = False
        
#     def resume_recording(self):
#         """恢复更新 Buffer"""
#         self.is_recording = True

#     def stop(self):
#         self.stop_event.set()
#         self.join()

# class RobotPolicySystem:
#     def __init__(self, action_space: ActionSpace = ActionSpace.JOINT_ANGLES, ip: str = "127.0.0.1", port: int = 6000):
#         self.action_space = action_space
        
#         # 初始化机器人
#         self.robot_env = FrankyEnv(
#             action_space=action_space, 
#             inference_mode=True, 
#             robot_param=RobotParam(np.array([ 0.0, 0.0, -math.pi / 2]), np.array([ 0.53433071, 0.52905707, 0.00440881]))
#         )
        
#         logging.info(f"Connecting to {ip}:{port}...")
#         self.client = TCPClientPolicy(host=ip, port=port)
#         logging.info("Connected.")
        
#         # 初始化相机
#         self.wrist_camera = RealSenseEnv(camera_name="wrist_image", serial_number="342222072092", width=1280, height=720)
#         self.recorder = ImageRecorder(self.wrist_camera, buffer_size=16)
        
#         # 夹爪状态记录
#         self.gripper_status = {"current_state": 0}
#         self.stop_evaluation = threading.Event()


#     def wait_until_still(self, velocity_threshold=0.05, required_stable_frames=3):
#         """
#         [死等模式] 阻塞程序，直到机械臂物理上完全静止。
#         """
#         t_start = time.time()
#         last_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
#         stable_counter = 0 
        
#         print(f"\n      🛑 Waiting for stop...")
        
#         while True:
#             time.sleep(0.05) 
#             curr_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
            
#             diff = np.abs(curr_pos - last_pos)
#             velocity = np.max(diff) / 0.05
            
#             if velocity < velocity_threshold:
#                 stable_counter += 1
#             else:
#                 stable_counter = 0
            
#             waited_time = time.time() - t_start
#             print(f"      ⏳ Speed: {velocity:.4f} | Stable: {stable_counter}/{required_stable_frames} | Time: {waited_time:.1f}s", end='\r')
            
#             if stable_counter >= required_stable_frames:
#                 print(f"                                                                      ", end='\r')
#                 print(f"      ✅ Stopped! (Waited: {waited_time:.2f}s)")
#                 return
            
#             last_pos = curr_pos

#     def run(self, task_name: str = "default_task"):
#         self.recorder.start()
#         logging.info("Waiting 2.0s for warmup...")
#         time.sleep(2.0)
        
#         EXECUTION_HORIZON = 15  
#         MAX_STEP_RAD = 0.05     # 调小限幅，用于平滑处理初始预测跳变
        
#         GRIPPER_THRESHOLD = 0.06 
        
#         logging.info(f"Starting SYNCHRONOUS inference loop... (Horizon: {EXECUTION_HORIZON})")

#         try:
#             while not self.stop_evaluation.is_set():
#                 if not self.recorder.is_alive(): break

#                 # 等到底层动作彻底跑完
#                 self.wait_until_still()

#                 # Phase 1: 准备推理 (Stop & Freeze)
#                 print("\n" + "="*50)
#                 print("[Phase 1] 🛑 Pausing Camera... (Freezing latest frame)")
#                 self.recorder.pause_recording()
                
#                 t0 = time.time()
                
#                 latest_img = self.recorder.get_latest_image()
#                 if latest_img is None:
#                     self.recorder.resume_recording()
#                     time.sleep(0.01)
#                     continue
                
#                 # 获取当前真实状态
#                 joint_angles = self.robot_env.get_position(action_space=ActionSpace.JOINT_ANGLES)
#                 gripper_width = self.robot_env.get_gripper_width()
#                 eef_pose = self.robot_env.get_position(action_space=ActionSpace.EEF_POSE)
                
#                 qpos_8d = list(joint_angles) + [float(gripper_width)]
#                 state = np.concatenate([eef_pose, [gripper_width]])
                
#                 element = {
#                     "observation/wrist_image": [latest_img],
#                     "observation/state": state,
#                     "qpos": qpos_8d, 
#                     "prompt": task_name,
#                 }

#                 # Phase 2: 推理
#                 print(f"[Phase 2] 🧠 Inferring...")
#                 inference_results = self.client.infer(element)
                
#                 infer_time = (time.time() - t0) * 1000
#                 print(f"          ✅ Inference Done in {infer_time:.1f}ms")
                
#                 # Phase 3: 执行 (Resume & Move)
#                 print("[Phase 3] ▶️ Resuming Camera & Executing Actions...")
#                 self.recorder.resume_recording()

#                 if inference_results and "actions" in inference_results:
#                     new_actions = np.array(inference_results["actions"][0])
#                     if len(new_actions) > 0:
#                         actions_to_execute = new_actions[:EXECUTION_HORIZON]

#                         for action in actions_to_execute:
#                             # 核心改进：直接使用模型预测的绝对目标角度，但在执行层做平滑限幅
#                             target_joints_raw = action[:7]
#                             gripper_val = action[7]

#                             # 获取机器人实时角度并计算限幅后的安全目标
#                             curr_joints = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
#                             diff = target_joints_raw - curr_joints
#                             limited_diff = np.clip(diff, -MAX_STEP_RAD, MAX_STEP_RAD)
#                             safe_target = curr_joints + limited_diff

#                             # 执行运动
#                             t_step_start = time.time()
#                             self.robot_env.step(safe_target, asynchronous=True)
                            
#                             # 夹爪逻辑
#                             if gripper_val > GRIPPER_THRESHOLD:
#                                 if self.gripper_status["current_state"] != -1:
#                                     logging.info(f"👐 Open ({gripper_val:.3f})")
#                                     self.robot_env.open_gripper(asynchronous=True)
#                                     self.gripper_status["current_state"] = -1
#                             elif gripper_val < GRIPPER_THRESHOLD:
#                                 if self.gripper_status["current_state"] != 1:
#                                     logging.info(f"✊ Close ({gripper_val:.3f})")
#                                     self.robot_env.close_gripper(asynchronous=True)
#                                     self.gripper_status["current_state"] = 1
                            
#                             # 控频 (25Hz - 30Hz)
#                             remain = 0.04 - (time.time() - t_step_start)
#                             if remain > 0: time.sleep(remain)

#                 latency = (time.time() - t0) * 1000
#                 print(f"\rStep Latency: {latency:.1f}ms", end="")

#         except KeyboardInterrupt:
#             logging.info("Keyboard Interrupt received.")
#         except Exception as e:
#             logging.error(f"Runtime Error: {e}")
#         finally:
#             self.stop()

#     def stop(self):
#         self.stop_evaluation.set()
#         if self.recorder.is_alive():
#             self.recorder.stop()
#         time.sleep(0.5)
#         logging.info("System stopped.")

# if __name__ == "__main__":
#     system = RobotPolicySystem(ip="127.0.0.1", port=6000)
#     system.run(task_name="pick up the orange ball and put it on the plank")


# # ego 二值化爪夹
# # inference/robot_policy_system.py
# import sys
# import os
# import time
# import logging
# import cv2
# import numpy as np
# import math
# import threading
# from collections import deque
# from common.constants import ActionSpace
# from robots.franky_env import FrankyEnv
# from robots.robot_param import RobotParam
# from systems.tcp_client import TCPClientPolicy 
# from cameras.realsense_env import RealSenseEnv

# # 配置日志格式
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# class ImageRecorder(threading.Thread):
#     def __init__(self, camera, buffer_size=16):
#         super().__init__()
#         self.camera = camera
#         self.buffer_size = buffer_size
#         self.running = False
#         self.lock = threading.Lock()
        
#         self.latest_frame = None
#         self.frame_buffer = deque(maxlen=buffer_size) 
#         self.stop_event = threading.Event()
        
#         # 录制控制标志位
#         self.is_recording = True 

#     def run(self):
#         self.running = True
#         self.camera.start_monitoring()
#         logging.info("[ImageRecorder] Background thread started.")
        
#         while not self.stop_event.is_set():
#             data = self.camera.get_latest_frame()
#             if data is not None:
#                 img = data['bgr']
                
#                 # 只有在“录制模式”下才更新 Buffer 和 Latest Frame
#                 if self.is_recording:
#                     with self.lock:
#                         self.latest_frame = img.copy()
#                         self.frame_buffer.append(img)
                
#                 # 实时显示保持常开
#                 cv2.imshow("Wrist View (Real-time)", img)
#                 if cv2.waitKey(1) & 0xFF == ord('q'):
#                     self.stop_event.set()
            
#             time.sleep(0.033) 
        
#         cv2.destroyAllWindows()
#         logging.info("[ImageRecorder] Stopped.")

#     def get_latest_image(self):
#         with self.lock:
#             if self.latest_frame is None: return None
#             return self.latest_frame.copy()

#     def pause_recording(self):
#         """暂停更新 Buffer"""
#         self.is_recording = False
        
#     def resume_recording(self):
#         """恢复更新 Buffer"""
#         self.is_recording = True

#     def stop(self):
#         self.stop_event.set()
#         self.join()

# class RobotPolicySystem:
#     def __init__(self, action_space: ActionSpace = ActionSpace.JOINT_ANGLES, ip: str = "127.0.0.1", port: int = 6000):
#         self.action_space = action_space
        
#         # 初始化机器人
#         self.robot_env = FrankyEnv(
#             action_space=action_space, 
#             inference_mode=True, 
#             robot_param=RobotParam(np.array([ 0.0, 0.0, -math.pi / 2]), np.array([ 0.53433071, 0.52905707, 0.00440881]))
#         )
        
#         logging.info(f"Connecting to {ip}:{port}...")
#         self.client = TCPClientPolicy(host=ip, port=port)
#         logging.info("Connected.")
        
#         # 初始化相机
#         self.wrist_camera = RealSenseEnv(camera_name="wrist_image", serial_number="342222072092", width=1280, height=720)
#         self.recorder = ImageRecorder(self.wrist_camera, buffer_size=16)
        
#         # 夹爪状态记录
#         self.gripper_status = {"current_state": 0}
#         self.stop_evaluation = threading.Event()


#     def wait_until_still(self, velocity_threshold=0.05, required_stable_frames=3):
#         """
#         [死等模式] 阻塞程序，直到机械臂物理上完全静止。
#         """
#         t_start = time.time()
#         last_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
#         stable_counter = 0 
        
#         print(f"\n      🛑 Waiting for stop...")
        
#         while True:
#             time.sleep(0.05) 
#             curr_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
            
#             diff = np.abs(curr_pos - last_pos)
#             velocity = np.max(diff) / 0.05
            
#             if velocity < velocity_threshold:
#                 stable_counter += 1
#             else:
#                 stable_counter = 0
            
#             waited_time = time.time() - t_start
#             print(f"      ⏳ Speed: {velocity:.4f} | Stable: {stable_counter}/{required_stable_frames} | Time: {waited_time:.1f}s", end='\r')
            
#             if stable_counter >= required_stable_frames:
#                 print(f"                                                                      ", end='\r')
#                 print(f"      ✅ Stopped! (Waited: {waited_time:.2f}s)")
#                 return
            
#             last_pos = curr_pos

#     def run(self, task_name: str = "default_task"):
#         self.recorder.start()
#         logging.info("Waiting 2.0s for warmup...")
#         time.sleep(2.0)
        
#         EXECUTION_HORIZON = 15  
#         MAX_STEP_RAD = 0.05     # 调小限幅，用于平滑处理初始预测跳变
        
#         # 🟢 [修改点] 调整夹爪阈值
#         # 模型输出: 1.0 (Open), -1.0 (Close)
#         # 所以阈值设为 0.0
#         GRIPPER_THRESHOLD = 0.0 
        
#         logging.info(f"Starting SYNCHRONOUS inference loop... (Horizon: {EXECUTION_HORIZON}, Grip Thresh: {GRIPPER_THRESHOLD})")

#         try:
#             while not self.stop_evaluation.is_set():
#                 if not self.recorder.is_alive(): break

#                 # 等到底层动作彻底跑完
#                 self.wait_until_still()

#                 # Phase 1: 准备推理 (Stop & Freeze)
#                 print("\n" + "="*50)
#                 print("[Phase 1] 🛑 Pausing Camera... (Freezing latest frame)")
#                 self.recorder.pause_recording()
                
#                 t0 = time.time()
                
#                 latest_img = self.recorder.get_latest_image()
#                 if latest_img is None:
#                     self.recorder.resume_recording()
#                     time.sleep(0.01)
#                     continue
                
#                 # 获取当前真实状态
#                 joint_angles = self.robot_env.get_position(action_space=ActionSpace.JOINT_ANGLES)
#                 gripper_width = self.robot_env.get_gripper_width()
#                 eef_pose = self.robot_env.get_position(action_space=ActionSpace.EEF_POSE)
                
#                 qpos_8d = list(joint_angles) + [float(gripper_width)]
#                 state = np.concatenate([eef_pose, [gripper_width]])
                
#                 element = {
#                     "observation/wrist_image": [latest_img],
#                     "observation/state": state,
#                     "qpos": qpos_8d, 
#                     "prompt": task_name,
#                 }

#                 # Phase 2: 推理
#                 print(f"[Phase 2] 🧠 Inferring...")
#                 inference_results = self.client.infer(element)
                
#                 infer_time = (time.time() - t0) * 1000
#                 print(f"          ✅ Inference Done in {infer_time:.1f}ms")
                
#                 # Phase 3: 执行 (Resume & Move)
#                 print("[Phase 3] ▶️ Resuming Camera & Executing Actions...")
#                 self.recorder.resume_recording()

#                 if inference_results and "actions" in inference_results:
#                     new_actions = np.array(inference_results["actions"][0])
#                     if len(new_actions) > 0:
#                         actions_to_execute = new_actions[:EXECUTION_HORIZON]

#                         for action in actions_to_execute:
#                             # 核心改进：直接使用模型预测的绝对目标角度，但在执行层做平滑限幅
#                             target_joints_raw = action[:7]
#                             gripper_val = action[7] # 此时为 1.0 或 -1.0

#                             # 获取机器人实时角度并计算限幅后的安全目标
#                             curr_joints = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
#                             diff = target_joints_raw - curr_joints
#                             limited_diff = np.clip(diff, -MAX_STEP_RAD, MAX_STEP_RAD)
#                             safe_target = curr_joints + limited_diff

#                             # 执行运动
#                             t_step_start = time.time()
#                             self.robot_env.step(safe_target, asynchronous=True)
                            
#                             # 夹爪逻辑 [保持结构，修改阈值]
#                             # 1.0 > 0.0 -> Open
#                             # -1.0 < 0.0 -> Close
#                             if gripper_val > GRIPPER_THRESHOLD:
#                                 if self.gripper_status["current_state"] != -1:
#                                     logging.info(f"👐 Open ({gripper_val:.1f})")
#                                     self.robot_env.open_gripper(asynchronous=True)
#                                     self.gripper_status["current_state"] = -1
#                             elif gripper_val < GRIPPER_THRESHOLD:
#                                 if self.gripper_status["current_state"] != 1:
#                                     logging.info(f"✊ Close ({gripper_val:.1f})")
#                                     self.robot_env.close_gripper(asynchronous=True)
#                                     self.gripper_status["current_state"] = 1
                            
#                             # 控频 (25Hz - 30Hz)
#                             remain = 0.04 - (time.time() - t_step_start)
#                             if remain > 0: time.sleep(remain)

#                 latency = (time.time() - t0) * 1000
#                 print(f"\rStep Latency: {latency:.1f}ms", end="")

#         except KeyboardInterrupt:
#             logging.info("Keyboard Interrupt received.")
#         except Exception as e:
#             logging.error(f"Runtime Error: {e}")
#         finally:
#             self.stop()

#     def stop(self):
#         self.stop_evaluation.set()
#         if self.recorder.is_alive():
#             self.recorder.stop()
#         time.sleep(0.5)
#         logging.info("System stopped.")

# if __name__ == "__main__":
#     system = RobotPolicySystem(ip="127.0.0.1", port=6000)
#     system.run(task_name="pick up the orange ball and put it on the plank")


# ego 双摄二值化
import sys
import os
import time
import logging
import cv2
import numpy as np
import math
import threading
from collections import deque
from common.constants import ActionSpace
from robots.franky_env import FrankyEnv
from robots.robot_param import RobotParam
from systems.tcp_client import TCPClientPolicy 
from cameras.realsense_env import RealSenseEnv

# 配置日志格式
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

class ImageRecorder(threading.Thread):
    def __init__(self, camera, name="Camera", buffer_size=16):
        super().__init__()
        self.camera = camera
        self.name = name
        self.buffer_size = buffer_size
        self.running = False
        self.lock = threading.Lock()
        
        self.latest_frame = None
        self.frame_buffer = deque(maxlen=buffer_size) 
        self.stop_event = threading.Event()
        self.is_recording = True 

    def run(self):
        self.running = True
        self.camera.start_monitoring()
        logging.info(f"[{self.name}] Background thread started.")
        
        while not self.stop_event.is_set():
            data = self.camera.get_latest_frame()
            if data is not None:
                img = data['bgr']
                
                if self.is_recording:
                    with self.lock:
                        self.latest_frame = img.copy()
                        self.frame_buffer.append(img)
                
                # 仅在主线程或专门的GUI线程显示，这里注释掉避免多线程GUI冲突
                # cv2.imshow(f"{self.name} View", img)
                # cv2.waitKey(1)
            
            time.sleep(0.033) # 30Hz
        
        logging.info(f"[{self.name}] Stopped.")

    def get_latest_image(self):
        with self.lock:
            if self.latest_frame is None: return None
            return self.latest_frame.copy()

    def pause_recording(self):
        self.is_recording = False
        
    def resume_recording(self):
        self.is_recording = True

    def stop(self):
        self.stop_event.set()
        self.join()

class RobotPolicySystem:
    def __init__(self, action_space: ActionSpace = ActionSpace.JOINT_ANGLES, ip: str = "127.0.0.1", port: int = 6000):
        self.action_space = action_space
        
        # 1. 初始化机器人
        self.robot_env = FrankyEnv(
            action_space=action_space, 
            inference_mode=True, 
            robot_param=RobotParam(np.array([ 0.0, 0.0, -math.pi / 2]), np.array([ 0.53433071, 0.52905707, 0.00440881]))
        )
        
        logging.info(f"Connecting to GPU Server {ip}:{port}...")
        self.client = TCPClientPolicy(host=ip, port=port)
        logging.info("Connected.")
        
        # 2. 初始化双相机
        # ⚠️ [重要] 请填写正确的序列号 (Serial Number)
        # 您可以使用 rs-enumerate-devices 查看
        WRIST_SERIAL = "342222072092"  
        MAIN_SERIAL  = "YOUR_MAIN_CAMERA_SERIAL" # <--- 请修改这里
        
        logging.info("Init Wrist Camera...")
        self.wrist_camera = RealSenseEnv(camera_name="wrist_image", serial_number=WRIST_SERIAL, width=640, height=480)
        self.wrist_recorder = ImageRecorder(self.wrist_camera, name="Wrist", buffer_size=16)
        
        # 如果没有主摄，可以临时注释掉或使用 Mock
        logging.info("Init Main Camera...")
        try:
            self.main_camera = RealSenseEnv(camera_name="main_image", serial_number=MAIN_SERIAL, width=640, height=480)
            self.main_recorder = ImageRecorder(self.main_camera, name="Main", buffer_size=16)
        except Exception as e:
            logging.warning(f"Main Camera Init Failed: {e}. using Dummy.")
            self.main_recorder = None

        self.gripper_status = {"current_state": 0}
        self.stop_evaluation = threading.Event()

    def wait_until_still(self, velocity_threshold=0.05, required_stable_frames=3):
        t_start = time.time()
        last_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
        stable_counter = 0 
        print(f"\n      🛑 Waiting for stop...")
        
        while True:
            time.sleep(0.05) 
            curr_pos = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
            diff = np.abs(curr_pos - last_pos)
            velocity = np.max(diff) / 0.05
            
            if velocity < velocity_threshold: stable_counter += 1
            else: stable_counter = 0
            
            if stable_counter >= required_stable_frames:
                print(f"      ✅ Stopped! (Waited: {time.time() - t_start:.2f}s)")
                return
            last_pos = curr_pos

    def run(self, task_name: str = "default_task"):
        self.wrist_recorder.start()
        if self.main_recorder: self.main_recorder.start()
        
        logging.info("Waiting 2.0s for warmup...")
        time.sleep(2.0)
        
        EXECUTION_HORIZON = 15  
        MAX_STEP_RAD = 0.05     
        GRIPPER_THRESHOLD = 0.0 # 二值化阈值 (Open=1.0, Close=-1.0)
        
        logging.info(f"Starting DUAL-VIEW inference loop...")

        try:
            while not self.stop_evaluation.is_set():
                # 检查线程存活
                if not self.wrist_recorder.is_alive(): break

                # 1. 等待机械臂静止 (Stop Phase)
                self.wait_until_still()

                # 2. 暂停录制 & 获取观测 (Snapshot)
                print("\n" + "="*50)
                print("[Phase 1] 📸 Capturing Dual Views...")
                self.wrist_recorder.pause_recording()
                if self.main_recorder: self.main_recorder.pause_recording()
                
                t0 = time.time()
                
                # 获取图像
                wrist_img = self.wrist_recorder.get_latest_image()
                if self.main_recorder:
                    main_img = self.main_recorder.get_latest_image()
                else:
                    # 如果没有主摄，造一个全黑图防止报错 (仅调试用)
                    main_img = np.zeros_like(wrist_img)

                if wrist_img is None or main_img is None:
                    print("⚠️ Image missing, retrying...")
                    self.wrist_recorder.resume_recording()
                    if self.main_recorder: self.main_recorder.resume_recording()
                    time.sleep(0.1)
                    continue
                
                # 获取状态
                joint_angles = self.robot_env.get_position(action_space=ActionSpace.JOINT_ANGLES)
                gripper_width = self.robot_env.get_gripper_width()
                eef_pose = self.robot_env.get_position(action_space=ActionSpace.EEF_POSE)
                
                qpos_8d = list(joint_angles) + [float(gripper_width)]
                state = np.concatenate([eef_pose, [gripper_width]])
                
                # 构造数据包 (关键：包含两个视角的Key)
                element = {
                    "observation/main_image": [main_img],   # List for sequence compatibility
                    "observation/wrist_image": [wrist_img],
                    "observation/state": state,
                    "qpos": qpos_8d, 
                    "prompt": task_name,
                }

                # 3. 发送推理请求
                print(f"[Phase 2] 🧠 Inferring (Main+Wrist)...")
                inference_results = self.client.infer(element)
                
                infer_time = (time.time() - t0) * 1000
                print(f"          ✅ Inference Done in {infer_time:.1f}ms")
                
                # 4. 执行动作 (Move Phase)
                print("[Phase 3] ▶️ Executing Actions...")
                self.wrist_recorder.resume_recording()
                if self.main_recorder: self.main_recorder.resume_recording()

                if inference_results and "actions" in inference_results:
                    new_actions = np.array(inference_results["actions"][0])
                    if len(new_actions) > 0:
                        actions_to_execute = new_actions[:EXECUTION_HORIZON]

                        for action in actions_to_execute:
                            target_joints_raw = action[:7]
                            gripper_val = action[7]

                            # 安全限幅平滑
                            curr_joints = np.array(self.robot_env.get_position(ActionSpace.JOINT_ANGLES))
                            diff = target_joints_raw - curr_joints
                            limited_diff = np.clip(diff, -MAX_STEP_RAD, MAX_STEP_RAD)
                            safe_target = curr_joints + limited_diff

                            t_step_start = time.time()
                            self.robot_env.step(safe_target, asynchronous=True)
                            
                            # 夹爪控制 (二值化)
                            if gripper_val > GRIPPER_THRESHOLD:
                                if self.gripper_status["current_state"] != -1:
                                    logging.info(f"👐 Open")
                                    self.robot_env.open_gripper(asynchronous=True)
                                    self.gripper_status["current_state"] = -1
                            elif gripper_val < GRIPPER_THRESHOLD:
                                if self.gripper_status["current_state"] != 1:
                                    logging.info(f"✊ Close")
                                    self.robot_env.close_gripper(asynchronous=True)
                                    self.gripper_status["current_state"] = 1
                            
                            # 控频 (25Hz)
                            remain = 0.04 - (time.time() - t_step_start)
                            if remain > 0: time.sleep(remain)

                latency = (time.time() - t0) * 1000
                print(f"\rStep Latency: {latency:.1f}ms", end="")

        except KeyboardInterrupt:
            logging.info("Stopped by user.")
        finally:
            self.stop()

    def stop(self):
        self.stop_evaluation.set()
        if self.wrist_recorder.is_alive(): self.wrist_recorder.stop()
        if self.main_recorder and self.main_recorder.is_alive(): self.main_recorder.stop()
        time.sleep(0.5)
        logging.info("System stopped.")

if __name__ == "__main__":
    # 请确保 IP 是 GPU 服务器地址
    system = RobotPolicySystem(ip="127.0.0.1", port=6000)
    system.run(task_name="pick up the orange ball and put it on the plank")




# rdt 单视角项目
# import sys
# import os
# import time
# import logging
# import cv2
# import numpy as np
# import math
# import threading
# from collections import deque
# from common.constants import ActionSpace
# from robots.franky_env import FrankyEnv
# from robots.robot_param import RobotParam
# # 确保 tcp_client.py 在 systems 文件夹下，或者根据实际位置修改 import
# from systems.tcp_client import TCPClientPolicy 
# from cameras.realsense_env import RealSenseEnv

# # 配置日志格式
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# class ImageRecorder(threading.Thread):
#     def __init__(self, camera, buffer_size=16):
#         super().__init__()
#         self.camera = camera
#         self.buffer_size = buffer_size
#         self.running = False
#         self.lock = threading.Lock()
        
#         self.latest_frame = None
#         self.frame_buffer = deque(maxlen=buffer_size) 
#         self.stop_event = threading.Event()

#     def run(self):
#         self.running = True
#         self.camera.start_monitoring()
#         logging.info("[ImageRecorder] Background thread started.")
        
#         while not self.stop_event.is_set():
#             data = self.camera.get_latest_frame()
#             if data is not None:
#                 img = data['bgr']
#                 with self.lock:
#                     self.latest_frame = img.copy()
#                     self.frame_buffer.append(img)
                
#                 # 实时显示，按 'q' 退出
#                 cv2.imshow("Wrist View (Real-time)", img)
#                 if cv2.waitKey(1) & 0xFF == ord('q'):
#                     self.stop_event.set()
            
#             # 保持约 30Hz 的采样率 (用于显示和缓存)
#             time.sleep(0.033) 
        
#         cv2.destroyAllWindows()
#         logging.info("[ImageRecorder] Stopped.")

#     def stop(self):
#         self.stop_event.set()
#         self.join()

# class RobotPolicySystem:
#     def __init__(self, action_space: ActionSpace = ActionSpace.JOINT_ANGLES, ip: str = "127.0.0.1", port: int = 6000):
#         self.action_space = action_space
        
#         # 初始化机器人
#         self.robot_env = FrankyEnv(
#             action_space=action_space, 
#             inference_mode=True, 
#             # 请确认这些初始姿态参数符合你的实际机器人
#             robot_param=RobotParam(np.array([ 0.0, 0.0, -math.pi / 2]), np.array([ 0.53433071, 0.52905707, 0.00440881]))
#         )
        
#         logging.info(f"Connecting to {ip}:{port}...")
#         self.client = TCPClientPolicy(host=ip, port=port)
#         logging.info("Connected.")
        
#         # 初始化相机
#         self.wrist_camera = RealSenseEnv(camera_name="wrist_image", serial_number="342222072092", width=1280, height=720)
#         self.recorder = ImageRecorder(self.wrist_camera, buffer_size=16)
        
#         # 夹爪状态记录: 0=未知, 1=闭合, -1=张开
#         self.gripper_status = {"current_state": 0}
#         self.stop_evaluation = threading.Event()

#     def run(self, task_name: str = "default_task"):
#         self.recorder.start()
#         logging.info("Waiting 2.0s for warmup...")
#         time.sleep(2.0)
        
#         # 每次推理执行步数 (Action Chunking)
#         EXECUTION_HORIZON = 15  
#         MAX_STEP_RAD = 0.08     # 关节动作限幅
#         last_executed_joints = None
        
#         # 夹爪动作阈值 (请根据实际情况微调)
#         GRIPPER_THRESHOLD = 0.06 
        
#         logging.info(f"Starting inference loop... (Gripper Threshold: {GRIPPER_THRESHOLD})")

#         try:
#             while not self.stop_evaluation.is_set():
#                 if not self.recorder.is_alive(): break

#                 t0 = time.time()
                
#                 # 1. 获取最新一帧图像
#                 with self.recorder.lock:
#                     latest_img = self.recorder.latest_frame
                
#                 if latest_img is None:
#                     time.sleep(0.01)
#                     continue
                
#                 # 包装成 List 发送
#                 wrist_images = [latest_img]

#                 # 2. 获取机器人状态
#                 joint_angles = self.robot_env.get_position(action_space=ActionSpace.JOINT_ANGLES)
#                 gripper_width = self.robot_env.get_gripper_width()
#                 eef_pose = self.robot_env.get_position(action_space=ActionSpace.EEF_POSE)
                
#                 # 拼装数据: 8维 (7关节 + 1夹爪)
#                 qpos_8d = list(joint_angles) + [float(gripper_width)]
#                 # state 用于 logging 或其他用途
#                 state = np.concatenate([eef_pose, [gripper_width]])
                
#                 element = {
#                     "observation/wrist_image": wrist_images, 
#                     "observation/state": state,
#                     "qpos": qpos_8d, 
#                     "prompt": task_name, # 发送任务指令
#                 }

#                 # 3. 发送推理请求
#                 inference_results = self.client.infer(element)
                
#                 if inference_results and "actions" in inference_results:
#                     new_actions = inference_results["actions"][0]
#                     if not isinstance(new_actions, list) or len(new_actions) == 0: continue

#                     # 截取前 EXECUTION_HORIZON 步执行
#                     actions_to_execute = new_actions[:EXECUTION_HORIZON]
#                     # print(f"  >>> Executing chunk ({len(actions_to_execute)} steps)...")

#                     for action in actions_to_execute:
#                         if not isinstance(action, (list, tuple, np.ndarray)): continue
                        
#                         action_np = np.array(action, dtype=np.float64)
#                         if np.all(action_np == 0) or np.isnan(action_np).any(): break
                        
#                         target_joints = action_np[:-1]
#                         gripper_val = action_np[-1]

#                         # 平滑限幅
#                         if last_executed_joints is not None:
#                             diff = np.clip(target_joints - last_executed_joints, -MAX_STEP_RAD, MAX_STEP_RAD)
#                             target_joints = last_executed_joints + diff
                        
#                         last_executed_joints = target_joints.copy()

#                         # 执行关节运动
#                         t_step_start = time.time()
#                         self.robot_env.step(target_joints, asynchronous=True)
                        
#                         # --- 夹爪控制逻辑 ---
#                         if gripper_val > GRIPPER_THRESHOLD:
#                             if self.gripper_status["current_state"] != -1:
#                                 logging.info(f"👐 [Gripper] OPEN ({gripper_val:.4f})")
#                                 self.robot_env.open_gripper(asynchronous=True)
#                                 self.gripper_status["current_state"] = -1
#                         elif gripper_val < GRIPPER_THRESHOLD:
#                             if self.gripper_status["current_state"] != 1:
#                                 logging.info(f"✊ [Gripper] CLOSE ({gripper_val:.4f})")
#                                 self.robot_env.close_gripper(asynchronous=True)
#                                 self.gripper_status["current_state"] = 1
#                         # ------------------
                        
#                         # [修改点] 控频调整
#                         # RDT 默认是 25Hz (dt=0.04s)。你之前的 0.025 (40Hz) 可能会让动作太快。
#                         # 如果你在 Server 端 inference_server.py 里设置了 --ctrl_freq 25，这里必须匹配。
#                         time_step = 0.04 
#                         remain = time_step - (time.time() - t_step_start)
#                         if remain > 0: time.sleep(remain)

#                 latency = (time.time() - t0) * 1000
#                 print(f"\rLoop Latency: {latency:.1f}ms | Gripper: {gripper_val:.4f}", end="")

#         except KeyboardInterrupt:
#             logging.info("Keyboard Interrupt received.")
#         except Exception as e:
#             logging.error(f"Runtime Error: {e}")
#         finally:
#             self.stop()

#     def stop(self):
#         self.stop_evaluation.set()
#         if self.recorder.is_alive():
#             self.recorder.stop()
#         time.sleep(0.5)
#         logging.info("System stopped.")

# if __name__ == "__main__":
#     # [修改点] 请将 IP 改为 GPU 服务器的实际 IP 地址
#     # 例如: system = RobotPolicySystem(ip="192.168.1.100", port=6000)
#     system = RobotPolicySystem(ip="127.0.0.1", port=6000)
    
#     # [修改点] 确保这里的指令与你训练时的一致
#     system.run(task_name="pick up the orange ball")


# # rdt 双视角项目
# import sys
# import os
# import time
# import logging
# import cv2
# import numpy as np
# import math
# import threading
# from collections import deque
# from common.constants import ActionSpace
# from robots.franky_env import FrankyEnv
# from robots.robot_param import RobotParam
# from systems.tcp_client import TCPClientPolicy 
# from cameras.realsense_env import RealSenseEnv

# # 配置日志
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# class ImageRecorder(threading.Thread):
#     def __init__(self, camera, name="Camera", buffer_size=16):
#         super().__init__()
#         self.camera = camera
#         self.name_tag = name
#         self.buffer_size = buffer_size
#         self.running = False
#         self.lock = threading.Lock()
        
#         self.latest_frame = None
#         self.frame_buffer = deque(maxlen=buffer_size) 
#         self.stop_event = threading.Event()

#     def run(self):
#         self.running = True
#         self.camera.start_monitoring()
#         logging.info(f"[{self.name_tag}] Background thread started.")
        
#         while not self.stop_event.is_set():
#             # 获取数据
#             data = self.camera.get_latest_frame()
#             if data is not None:
#                 img = data['bgr']
#                 with self.lock:
#                     self.latest_frame = img.copy()
#                     self.frame_buffer.append(img)
                
#                 # ❌ 删除这里的 imshow，它不能在子线程运行
#                 # cv2.imshow(...) 
            
#             # 保持 30Hz 采样
#             time.sleep(1.0 / 30.0) 
        
#         logging.info(f"[{self.name_tag}] Stopped.")

#     def stop(self):
#         self.stop_event.set()
#         self.join()

# class RobotPolicySystem:
#     def __init__(self, action_space: ActionSpace = ActionSpace.JOINT_ANGLES, ip: str = "127.0.0.1", port: int = 6000):
#         self.action_space = action_space
        
#         # 1. 初始化机器人
#         self.robot_env = FrankyEnv(
#             action_space=action_space, 
#             inference_mode=True, 
#             robot_param=RobotParam(np.array([ 0.0, 0.0, -math.pi / 2]), np.array([ 0.53433071, 0.52905707, 0.00440881]))
#         )
        
#         logging.info(f"Connecting to {ip}:{port}...")
#         self.client = TCPClientPolicy(host=ip, port=port)
#         logging.info("Connected.")
        
#         # 2. 初始化双相机
#         # ⚠️ 请确保这里填入了正确的序列号
#         logging.info("Initializing Cameras...")
#         self.wrist_camera = RealSenseEnv(
#             camera_name="wrist_image", 
#             serial_number="YOUR_WRIST_SERIAL_NUMBER", 
#             width=640, height=480
#         )
#         self.wrist_recorder = ImageRecorder(self.wrist_camera, name="Wrist", buffer_size=16)

#         self.head_camera = RealSenseEnv(
#             camera_name="head_image", 
#             serial_number="YOUR_HEAD_SERIAL_NUMBER", 
#             width=640, height=480
#         )
#         self.head_recorder = ImageRecorder(self.head_camera, name="Head", buffer_size=16)

#         self.gripper_status = {"current_state": 0}
#         self.stop_evaluation = threading.Event()

#     def run(self, task_name: str = "default_task"):
#         # 启动录制线程
#         self.wrist_recorder.start()
#         self.head_recorder.start()
        
#         logging.info("Waiting 2.0s for warmup...")
#         time.sleep(2.0)
        
#         EXECUTION_HORIZON = 15  
#         MAX_STEP_RAD = 0.08     
#         GRIPPER_THRESHOLD = 0.06 
#         last_executed_joints = None
        
#         logging.info(f"Starting inference loop... (Gripper Threshold: {GRIPPER_THRESHOLD})")

#         try:
#             while not self.stop_evaluation.is_set():
#                 if not self.wrist_recorder.is_alive() or not self.head_recorder.is_alive(): 
#                     logging.error("Camera thread died!")
#                     break

#                 t0 = time.time()
                
#                 # 1. 获取图像
#                 with self.wrist_recorder.lock:
#                     wrist_img = self.wrist_recorder.latest_frame
#                 with self.head_recorder.lock:
#                     head_img = self.head_recorder.latest_frame
                
#                 # 2. ✅ [核心修复] 在主线程进行显示
#                 # 只有在主线程调用 imshow 才能正常刷新窗口
#                 display_ok = True
#                 if wrist_img is not None:
#                     cv2.imshow("Wrist View", wrist_img)
#                 else:
#                     display_ok = False
                    
#                 if head_img is not None:
#                     cv2.imshow("Head View", head_img)
#                 else:
#                     display_ok = False
                
#                 # 必须有 waitKey 才能刷新 GUI
#                 if cv2.waitKey(1) & 0xFF == ord('q'):
#                     break
                
#                 # 如果任意一个相机还没图，就先跳过推理，等待相机准备好
#                 if not display_ok:
#                     time.sleep(0.01)
#                     continue

#                 # 3. 获取状态
#                 joint_angles = self.robot_env.get_position(action_space=ActionSpace.JOINT_ANGLES)
#                 gripper_width = self.robot_env.get_gripper_width()
#                 eef_pose = self.robot_env.get_position(action_space=ActionSpace.EEF_POSE)
                
#                 qpos_8d = list(joint_angles) + [float(gripper_width)]
#                 state = np.concatenate([eef_pose, [gripper_width]])
                
#                 # 4. 构造请求
#                 element = {
#                     "observation/head_image": [head_img],   
#                     "observation/wrist_image": [wrist_img], 
#                     "observation/state": state,
#                     "qpos": qpos_8d, 
#                     "prompt": task_name,
#                 }

#                 # 5. 推理
#                 inference_results = self.client.infer(element)
                
#                 if inference_results and "actions" in inference_results:
#                     new_actions = inference_results["actions"][0]
#                     if not isinstance(new_actions, list) or len(new_actions) == 0: continue

#                     actions_to_execute = new_actions[:EXECUTION_HORIZON]

#                     for action in actions_to_execute:
#                         if not isinstance(action, (list, tuple, np.ndarray)): continue
                        
#                         action_np = np.array(action, dtype=np.float64)
#                         target_joints = action_np[:-1]
#                         gripper_val = action_np[-1]

#                         if last_executed_joints is not None:
#                             diff = np.clip(target_joints - last_executed_joints, -MAX_STEP_RAD, MAX_STEP_RAD)
#                             target_joints = last_executed_joints + diff
#                         last_executed_joints = target_joints.copy()

#                         t_step_start = time.time()
#                         self.robot_env.step(target_joints, asynchronous=True)
                        
#                         if gripper_val > GRIPPER_THRESHOLD:
#                             if self.gripper_status["current_state"] != -1:
#                                 self.robot_env.open_gripper(asynchronous=True)
#                                 self.gripper_status["current_state"] = -1
#                         elif gripper_val < GRIPPER_THRESHOLD:
#                             if self.gripper_status["current_state"] != 1:
#                                 self.robot_env.close_gripper(asynchronous=True)
#                                 self.gripper_status["current_state"] = 1
                        
#                         # 30Hz 控频
#                         time_step = 1.0 / 30.0 
#                         remain = time_step - (time.time() - t_step_start)
#                         if remain > 0: time.sleep(remain)

#                 latency = (time.time() - t0) * 1000
#                 print(f"\rLatency: {latency:.1f}ms | Gripper: {gripper_val:.4f}", end="")

#         except KeyboardInterrupt:
#             logging.info("Keyboard Interrupt received.")
#         except Exception as e:
#             logging.error(f"Runtime Error: {e}")
#             import traceback
#             traceback.print_exc()
#         finally:
#             self.stop()
#             cv2.destroyAllWindows() # 退出时关闭窗口

#     def stop(self):
#         self.stop_evaluation.set()
#         if self.wrist_recorder.is_alive(): self.wrist_recorder.stop()
#         if self.head_recorder.is_alive(): self.head_recorder.stop()
#         time.sleep(0.5)
#         logging.info("System stopped.")

# if __name__ == "__main__":
#     # 请确保修改 IP 和上面的相机 Serial Number
#     system = RobotPolicySystem(ip="127.0.0.1", port=6000)
#     system.run(task_name="pick up the orange ball and put it on the plank")