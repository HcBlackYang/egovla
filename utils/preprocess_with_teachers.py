...
def save_to_hdf5(raw_dir, output_path, siglip_path):
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        for i, ep_path in enumerate(tqdm(episodes)):
            ...
            # 动态计算 end_idx
            if gripper_width is not None and len(gripper_width) > 0:
                # 计算当前轨迹的最大宽度（假设这是完全打开的状态）
                max_width = np.max(gripper_width)
                # 设置阈值：比最大值小一点点，容忍传感器噪声 (例如 2mm)
                open_threshold = max_width - 0.002
                
                # 默认值：如果不满足条件，则不切除尾部（或者你可以设置为切除固定90帧）
                end_idx = total_len
                
                # 倒序遍历：寻找最后一次“未完全打开”的时刻
                # 意思是：找到 gripper_width < threshold 的最后位置
                # 这样可以确保我们切在 "动作完成并松开" 的时刻之后
                for t in range(total_len - 1, -1, -1):
                    if gripper_width[t] < open_threshold:
                        # found it! t 是最后一次处于闭合/半闭合状态
                        # t + 1 就是开始变为完全打开的那一帧
                        # 用户要求：完全打开后 10 帧
                        end_idx = (t + 1) + 10
                        break
                ...
                # 如果没有 gripper 数据，回退到原来的逻辑 (尾部切90)
                end_idx = total_len - 90
                print(f"没有发现 gripper 数据，使用默认尾部裁剪策略。")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--raw_dir', type=str, default='/yanghaochuan/data/pick_up_the_orange_ball', help='原始数据目录')
    parser.add_argument('--out_path', type=str, default='/yanghaochuan/data/12pick_up_the_orange_ball.hdf5')
    parser.add_argument('--siglip_path', type=str, default='/yanghaochuan/models/siglip-so400m-patch14-384')
